...
require '/var/www/html/G-monde/vendor/autoload.php';  // Assurez-vous que ce chemin est correct
...
// Fonction pour envoyer un SMS avec Vonage
function envoyerSMS($message) {
    $basic = new VonageBasic("4dd607d3", "0n9hPJemHh0bgpPN");
    $client = new VonageClient($basic);

    $response = $client->sms()->send(
        new \Vonage\SMS\Message\SMS("221783830020", 'GP-CARGO', $message)
    );
        ...
        if ($cargo['numero'] === $numero) {
            if ($statut) {
                if ($statut === 'Arrivé') {
                    ...
                    envoyerSMS('Votre cargaison est maintenant en cours.');
                } else {
